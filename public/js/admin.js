...
/* ===== PRÉVIA AO VIVO DO ANÚNCIO NO EDITOR ===== */

/* ===== NORMALIZAÇÃO DE URL DAS IMAGENS DO ADMIN ===== */
function urlImagemAdmin(caminho) {
    ...
    if (caminho.startsWith("https://res.cloudinary.com/")) {
        ...
        if (posicao >= 0) {
            ...
            // Remove transformações do Cloudinary para usar a imagem original pelo proxy interno.
...
function obterImagemPreviaProduto() {
    const principal = document.querySelector(".cor-imagem-principal:checked");
    if (principal) {
        const item = principal.closest(".imagem-item");
        const img = item ? item.querySelector("img") : null;
        if (img && (img.currentSrc || img.src)) return img.currentSrc || img.src;
    }

    const primeira = document.querySelector(".imagem-item img");
    return primeira && primeira.src ? primeira.src : "";
}
...
function atualizarPreviaProduto() {
    ...
    const titulo = document.getElementById("f-titulo")?.value.trim() || "Seu produto";
    const linha = document.getElementById("f-linha")?.value.trim() || "";
    const preco = document.getElementById("f-preco")?.value || "";
    const parcelamento = document.getElementById("f-parcelamento")?.value.trim() || "";
    const categoriaSelect = document.getElementById("f-categoria");
    const categoria = categoriaSelect?.selectedOptions?.[0]?.value
        ? (categoriaSelect.selectedOptions[0].textContent || "")
        : "";
    const imgSrc = obterImagemPreviaProduto();

    card.innerHTML =
        '<div class="previa-imagem-area">' +
            (imgSrc
                ? '<img src="' + escaparHtml(imgSrc) + '" alt="' + escaparHtml(titulo) + '">' 
                : '<div class="previa-placeholder"><strong>SEM IMAGEM</strong><span>Adicione a foto principal</span></div>') +
            '<span class="previa-favorito" aria-hidden="true">♡</span>' +
        '</div>' +
        '<div class="previa-info">' +
            (linha
                ? '<div class="previa-categoria">' + escaparHtml(linha) + '</div>'
                : (categoria ? '<div class="previa-categoria">' + escaparHtml(categoria) + '</div>' : '')) +
            '<div class="previa-nome">' + escaparHtml(titulo) + '</div>' +
            (preco
                ? '<div class="previa-preco">' + escaparHtml(formatarPreco(preco)) + '</div>'
                : '<div class="previa-preco" style="color:#9aa5b1;font-size:.88rem">Preço não informado</div>') +
            (parcelamento ? '<div class="previa-parcelas">' + escaparHtml(parcelamento) + '</div>' : '') +
            '<div class="previa-detalhes">Ver detalhes</div>' +
        '</div>';
}
...
function iniciarPreviaProduto() {
    ...
    if (modal) {
        const observerModal = new MutationObserver(() => {
            if (modal.classList.contains("aberto")) atualizarPreviaProduto();
        });
